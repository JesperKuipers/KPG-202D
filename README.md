...
- Wrote a [PowerShell script](#) that opened a hidden feature called **Password reset execution**, but it was password protected  
...
- **Release v1** = patched piracy protection, is still password-locked but doesn't throw an error when changing the license key in registry  
...
- First patch removed the error dialog but stopped execution → useless (Release v2.0)  
...
- Finally found the correct patch → **any password accepted** 🎉 (Release v2.5)  
...
- After few more patches → worked after any password attempt (Release v3)  
...
- **Release v4** accepts any password (or blank) for **both read & write**  
